	...
	<figure class="woocommerce-product-gallery__wrapper">
		...
		if ( has_post_thumbnail() ) {
			$image = nanofit_get_image_resized( array(
				...
				'crop'     => nanofit_option( 'product__imageSizeCrop' ),
				'atts'     => $attributes
			) );

			$thumbnail       = nanofit_get_image_resized( array(
				'image_id' => $post_thumbnail_id,
				'size'     => nanofit_option( 'product__thumbnailSize' ),
				'crop'     => nanofit_option( 'product__thumbnailSizeCrop' )
			) );
			...
			$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_html__( 'Awaiting product image', 'nanofit' ) );
			$html .= '</div>';
		}
