...
class Nanofit_Custom_Sidebars
{
	...
	public function enqueue_assets( $hook = null ) {
		...
		wp_enqueue_style( 'nanofit-custom-sidebars', get_theme_file_uri( 'admin/css/sidebars.css' ) );
		wp_enqueue_script( 'nanofit-custom-sidebars', get_theme_file_uri( 'admin/js/sidebars.js' ) );
		...
		wp_localize_script( 'nanofit-custom-sidebars', '_sidebarSettings', array(
			'button_title'    => esc_html__( 'remove this area', 'nanofit' ),
			'confirm_message' => esc_html__( 'Are you sure you want to remove this widget area?', 'nanofit' ),
	...
	public function register_sidebars() {
		...
		$sidebar_options = apply_filters( 'nanofit/custom_sidebars_params', array(
	...
	public function admin_form() {
		...
		$messages = array();
		$messages[1] = esc_html__( 'New custom widget area has been created', 'nanofit' );
		$messages[2] = esc_html__( 'Custom widget area has been removed', 'nanofit' );

		$errors = array();
		$errors[2] = esc_html__( 'Cannot create custom widget area, please try again', 'nanofit' );
		$errors[3] = esc_html__( 'Invalid the ID of widget area', 'nanofit' );
		?>
			...
			<div id="sidebars-form" class="widget-liquid-right">
				<form action="<?php echo esc_url( admin_url() ) ?>admin-ajax.php?action=save_custom_sidebar" method="POST" class="widgets-holder-wrap">
					...
					<h3><?php esc_html_e( 'Create Widget Area', 'nanofit' ) ?></h3>
					<div class="sidebar-inputs">
						<label for="sidebar-name"><?php esc_html_e( 'Enter name of the widget area:', 'nanofit' ) ?></label>
						<input type="text" name="sidebar-name" id="sidebar-name" />
						<input type="submit" class="button button-primary" value="<?php esc_html_e( 'Create', 'nanofit' ) ?>" />
					</div>
				</form>
				...
				<?php if ( ! empty( $custom_sidebars ) ): ?>
					<form id="remove-sidebar-form" action="<?php echo esc_url( admin_url() ) ?>admin-ajax.php" method="GET" class="widgets-holder-wrap hide-if-js">
						...
						<h3><?php esc_html_e( 'Remove Custom Widget Area', 'nanofit' ) ?></h3>
						<div class="sidebar-inputs">
							<label for="sidebar-id"><?php esc_html_e( 'Select custom widget area to be removed:', 'nanofit' ) ?></label>
							...
							<input type="submit" class="button" value="<?php esc_html_e( 'Remove', 'nanofit' ) ?>" />
						</div>
					</form>
				<?php endif ?>
			</div>
	...
	public function save_sidebar() {
		if ( isset( $_POST['custom-sidebar'] ) &&
			...
			$custom_sidebars['sidebar-' . uniqid()] = array(
				'name' => $name,
				'description' => sprintf( esc_html__( 'There are widgets for %s', 'nanofit' ), $name )
			);
