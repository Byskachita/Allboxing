 ...
 * @return  mixed
 */
function nanofit_option( $id ) {
	static $theme_settings;

	if ( empty( $theme_settings ) ) {
		$theme_settings = apply_filters( 'nanofit_customize_settings', array() );
	}
 ...
 * @return  strings
 */
function nanofit_current_post_type() {
 ...
 * @since   1.0.0
 */
function nanofit_logo( $prefix = '', $attrs = array() ) {
	$logo_key = 'logo';
	...
	$logo = (array) nanofit_option( $logo_key );
	$logo_retina = (array) nanofit_option( $logo_retina_key );
	...
	printf( '<img %s />', nanofit_attributes( $attributes ) );
}


function nanofit_social_icons( $args ) {
	...
	$positions = nanofit_option( 'global__social__positions' );

	if ( in_array( $args['location'], $positions ) ) {
		$icons = nanofit_option( 'global__social__icons' );
		$output = array();
 ...
 * @since   1.0.0
 */
function nanofit_attributes( $attrs ) {
	$attributes = array();
 ...
 * @since   1.0.0
 */
function nanofit_google_fonts() {
	// Try to retrive the font list from the cache
	$cached = get_transient( 'nanofit-fonts' );

	if ( ! $cached ) {
		$remote_content = wp_safe_remote_get( 'https://demo.21lab.co/resources/fonts.php', array(
			'timeout' => 9999
		) );
		...
		if ( is_array( $remote_content ) && isset( $remote_content['body'] ) ) {
			...
			set_transient( 'nanofit-fonts', $cached, 86400 );
		}
	}
 ...
 * @since   1.0.0
 */
function nanofit_initialize_filesystem_api( $url = '', $action = -1, $name = '_wpnonce' ) {
 ...
 * @return  array
 */
function nanofit_get_image_resized( $params = array() ) {
	...
	if ( is_string( $thumb_size ) && ( ( ! empty( $_wp_additional_image_sizes[ $thumb_size ] ) && is_array( $_wp_additional_image_sizes[ $thumb_size ] ) ) || in_array( $thumb_size, array(
		...
		if ( is_array( $thumb_size ) ) {
			...
			$p_img = nanofit_image_resize( $attach_id, null, $thumb_size[0], $thumb_size[1], $params['crop'] );
			...
			if ( ! empty( $attachment ) ) {
				...
				if ( $p_img ) {
					...
					$thumbnail = '<img ' . nanofit_attributes_stringify( $attributes ) . ' />';
				}
			}
		}
	} else {
	...
	return apply_filters( 'nanofit_get_image_resized', array(
		'thumbnail' => $thumbnail,
		'thumbnail_raw' => $p_img,
		'large' => $p_img_large,
	), $attach_id, $params );
 ...
 * @return array
 */
function nanofit_image_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
 ...
 * @return string
 */
function nanofit_attributes_stringify( $attributes ) {
	$atts = array();
 ...
 */
function nanofit_get_image_sizes() {
	global $_wp_additional_image_sizes;
